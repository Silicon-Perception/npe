# NPP SDK - 基于 NPE 架构的高性能网络通信引擎

> 纯 C 语言实现的开源网络通协议栈，用增量传输替代全量传输范式，大幅降低计算和带宽开销。
...
### 传统网络通
![传统通](images/a1.gif)
- **全量传输**：每帧传输全部数据，即使 99% 数据未变化
- **周期心跳**：设备空闲时仍持续发送保活包，消耗带宽和功耗
- **计算密集**：CPU 参与数据组包/拆包、校验/重传

### NPP 网络通
![NPP 通](images/a2.gif)
- **增量传输**：仅传输变化的管道数据；静态时零带宽
- **沉默即证明**：无数据变化 = 设备在线，无需心跳
- **比较驱动**：核心操作仅比较和赋值，无乘除法
...
|------|-----------|-----|------|
| 静态（无变化） | ~1KB/s | 0 | **~100%** |
| 低活跃（10% 管道） | ~1KB/s | ~50B/s | **95%** |
| 高活跃（全部管道） | ~1KB/s | ~400B/s | **60%** |
